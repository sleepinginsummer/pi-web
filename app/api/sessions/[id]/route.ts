...
export async function DELETE(
  ...
  try {
    ...
    // 保留本地回收站语义；子代理后代也必须从活动目录移走，避免孤儿会话。
    let trashedName = "";
    for (const [deletedId, deletedPath] of deletedPaths) {
      try {
        const name = trashSessionFile(deletedPath);
        if (deletedId === id) trashedName = name;
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
      }
    ...
    try {
      await removeSidebarPreferenceIds({ sessionIds: [...deletedSessionIds] });
    } catch (error) {
      console.error("清理已删除会话的侧边栏偏好失败", error);
    }
    invalidateSessionListCache();
    if (trashedName) queueTrashSessionTitle(trashedName);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json({ error: String(error) }, { status: 500 });
  }
