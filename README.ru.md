...
PI_WEB_PASSWORD='длинный-случайный-пароль' pi-web  # страница входа (имя пользователя: pi)
PI_WEB_NO_OPEN=1 pi-web         # удобно для фоновой службы
```

Задайте `PI_WEB_PASSWORD`, чтобы защитить веб-интерфейс и все конечные точки API страницей входа. Имя пользователя всегда `pi`; если переменная не задана или пуста, аутентификация отключена. После входа используется сессионный файл cookie с флагом `HttpOnly`; при выборе сохранения входа он действует 30 дней. Секрет подписи хранится в `~/.pi/agent/pi-web-session.json` с доступом только для текущего пользователя. При переключении сетей используйте одно имя хоста, поскольку браузеры разделяют cookie по хостам.

Pi Web может вызывать агента с широкими полномочиями. Не публикуйте его напрямую в интернете; для удалённого доступа используйте HTTPS через доверенный обратный прокси или надёжное VPN-подключение.
