...
test("does not expose empty widgets in the runtime snapshot", async () => {
  const { context, wrapper } = createContext();

  context.setWidget("empty", []);
  assert.deepEqual(await snapshot(wrapper), []);

  context.setWidget("empty", ["visible"]);
  assert.deepEqual(await snapshot(wrapper), [{ key: "empty", lines: ["visible"], placement: "aboveEditor" }]);
  context.setWidget("empty", []);
  assert.deepEqual(await snapshot(wrapper), []);
