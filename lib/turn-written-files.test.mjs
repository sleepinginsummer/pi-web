...
test("extracts a file from Pi's successful replace tool call", () => {
  const content = [toolCall("1", "replace", { path: "/abs/src/a.ts" })];
  assert.deepEqual(paths(content, results(okResult("1"))), ["/abs/src/a.ts"]);
});
...
test("apply_patch falls back to preview newPath and ignores planned deletes", () => {
  const content = [applyPatchCall("1", MULTI_FILE_PATCH)];
  const result = applyPatchResult("1", {
    preview: {
      files: [
  ...
  assert.deepEqual(paths(content, results(result), "/repo"), ["/repo/src/a.ts"]);
});
