...
test("显式 provider 模型配置会排除该 provider 的内置目录余项", async () => {
  const scope = await resolveVisibleModels(runtime, undefined);
  const restricted = restrictVisibleModelsToConfiguredCatalog(scope, {
    providers: {
      anthropic: {
        models: [
          { id: "claude-opus-5" },
  ...
  assert.deepEqual(refs(restricted), [
...
test("未显式声明 models 的 provider 保持 Pi 目录行为", async () => {
  const scope = await resolveVisibleModels(runtime, ["anthropic/*:high"]);
  const unchanged = restrictVisibleModelsToConfiguredCatalog(scope, {
    providers: { anthropic: { baseUrl: "https://api.example.com" } },
  });

  assert.equal(unchanged, scope);
});

test("配置收口会同步移除 scopedModels 和思考等级 pin", async () => {
  const scope = await resolveVisibleModels(runtime, ["anthropic/*:high"]);
  const restricted = restrictVisibleModelsToConfiguredCatalog(scope, {
    providers: { anthropic: { models: [{ id: "claude-opus-5" }] } },
  });

  assert.deepEqual(refs(restricted), ["anthropic/claude-opus-5"]);
  assert.deepEqual(
    restricted.scopedModels.map(({ model }) => `${model.provider}/${model.id}`),
    ["anthropic/claude-opus-5"],
  );
  assert.deepEqual(restricted.thinkingLevelPins, { "anthropic/claude-opus-5": "high" });
});
