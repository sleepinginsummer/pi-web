...
test("sliceActiveBranch walks from leaf back toward the current turn anchor", () => {
  const entries = linearChain(10);
  const sliced = sliceActiveBranch(entries, "e5", 3);
  assert.deepEqual(sliced.map((e) => e.id), ["e2", "e3", "e4", "e5"]);
});

test("tail pagination keeps a long tool-calling turn anchored by its user message", () => {
  const entries = [
  ...
  assert.equal(context.entryIds[0], "current-user");
