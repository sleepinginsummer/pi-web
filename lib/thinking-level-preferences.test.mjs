...
test("每个推荐只考虑全局记录中最近 10 次同模型用法", () => {
  const usages = [
    ...Array.from({ length: 40 }, () => ({ modelId: "model-a", thinkingLevel: "high" })),
    ...Array.from({ length: 10 }, () => ({ modelId: "model-a", thinkingLevel: "max" })),
  ];
  assert.equal(recommendThinkingLevel(usages, "model-a"), "max");
});
...
test("每个模型独立保留最近 10 次记录", async () => {
  await withPreferenceFile(async (filePath) => {
    ...
    const saved = JSON.parse(await readFile(filePath, "utf8"));
    assert.equal(saved.models["model-a"].length, 10);
