...
test("defers historical base64 images from every message role", () => {
  const userImage = {
    type: "image",
  ...
  const deferred = buildSessionContext(entries, undefined, { deferToolResultImages: true, sessionId: "session" });
  assert.match(deferred.messages[0].content[1].source.url, /u1\/tool-result-image\?blockIndex=1/);
  assert.deepEqual(deferred.messages[2].content[0], { type: "text", text: "Read image file" });
  assert.match(deferred.messages[2].content[1].source.url, /tr1\/tool-result-image\?blockIndex=1/);
  assert.match(deferred.messages[2].content[2].source.url, /tr1\/tool-result-image\?blockIndex=2/);
...
test("renders Shadow Mind lifecycle events as visible custom cards", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      type: "custom",
      ...
      data: { kind: "heartbeat", data: { activated: [] } },
    },
    {
      type: "custom",
      id: "run-start",
      ...
      data: {
        kind: "run-start",
        data: { runId: "shadow-run-1", shadowId: "code-smell-auditor", model: "test/gpt" },
      },
    },
    {
      type: "custom",
      ...
      data: {
        kind: "run-end",
        data: { runId: "shadow-run-1", shadowId: "code-smell-auditor", thinkingLevel: "high", reason: "silent", durationMs: 4212 },
      },
    },
    {
      type: "custom",
      ...
      customType: "shadow-mind-event",
      data: { kind: "runs-aborted", data: { count: 0, reason: "session-shutdown" } },
    },
    {
      type: "custom",
      id: "abort",
      parentId: "empty-abort",
      timestamp: "2026-01-01T00:00:05.000Z",
      customType: "shadow-mind-event",
      data: { kind: "runs-aborted", data: { count: 2, reason: "new-user-input" } },
    },
    assistantEntry("a1", "abort", "done"),
  ];

  const context = buildSessionContext(entries);

  assert.deepEqual(context.entryIds, ["u1", "run-start", "run-end", "abort", "a1"]);
  assert.equal(context.messages[1].role, "custom");
  assert.equal(context.messages[1].customType, "shadow-mind");
  assert.equal(context.messages[1].content, "code-smell-auditor");
  assert.deepEqual(context.messages[1].details, {
    event: "run-start",
    shadowId: "code-smell-auditor",
    model: "test/gpt",
    thinkingLevel: "high",
  });
  assert.deepEqual(context.messages[2].details, {
    event: "run-end",
    shadowId: "code-smell-auditor",
    model: "test/gpt",
    thinkingLevel: "high",
  ...
  assert.deepEqual(context.messages[3].details, {
    event: "runs-aborted",
    count: 2,
...
test("updates cached project metadata without creating a session-list cache", () => {
  globalThis.__piSessionListCache = undefined;
  globalThis.__piProjectMetadataRevision = 0;
  updateCachedSessionProject("/repo", {
    projectRoot: "/repo",
    branch: "main",
    isWorktree: false,
    isTopLevel: true,
  ...
  assert.equal(globalThis.__piProjectMetadataRevision, 1);

  const untouched = { id: "other", cwd: "/other", currentBranch: "old" };
  const target = { id: "target", cwd: "/repo", currentBranch: "old", projectRoot: "/old" };
  globalThis.__piSessionListCache = { data: [untouched, target], ts: 123 };
  updateCachedSessionProject("/repo", {
    projectRoot: "/repo",
    branch: "main",
    isWorktree: false,
  ...
  assert.equal(globalThis.__piSessionListCache.ts, 123);
  assert.equal(globalThis.__piSessionListCache.data[0], untouched);
  assert.deepEqual(globalThis.__piSessionListCache.data[1], {
    ...target,
    projectRoot: "/repo",
    currentBranch: "main",
    isWorktree: false,
  ...
  assert.equal(globalThis.__piProjectMetadataRevision, 3);
  globalThis.__piSessionListCache = undefined;
  globalThis.__piProjectMetadataRevision = undefined;
});
