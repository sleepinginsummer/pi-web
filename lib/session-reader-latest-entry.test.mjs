...
test("超出 64 KB 的完整末尾记录仍能识别外部写入", (t) => {
  const oversized = { ...message("external-large"), message: { role: "user", content: "中".repeat(30_000) } };
  const file = withSessionFile(t, lines(header, message("known"), oversized));
  assert.equal(readLatestSessionEntryId(file), "external-large");
});

test("超长撕裂记录不会掩盖上一条完整记录", (t) => {
  const file = withSessionFile(t, `${lines(header, message("known"))}{"id":"unfinished","content":"${"x".repeat(90_000)}`);
  assert.equal(readLatestSessionEntryId(file), "known");
});
