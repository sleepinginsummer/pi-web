...
const sessionItemSource = await readFile(new URL("./SessionItem.tsx", import.meta.url), "utf8");
...
test("uses the child-session robot glyph for the sub-agents tab", () => {
  ...
  assert.match(sessionItemSource, robotGlyph);
...
test("保留本地用户名、密码和退出入口", () => {
  const formSource = readFile(new URL("../app/login/LoginForm.tsx", import.meta.url), "utf8");
  return formSource.then((form) => {
    assert.match(form, /name="username"/);
    assert.match(form, /type="password"/);
    assert.match(form, /autoComplete="current-password"/);
    assert.match(form, /getSafeInternalPath\(searchParams\.get\("next"\)\)/);
    assert.match(panelSource, /fetch\("\/api\/web-auth\/session", \{ method: "DELETE" \}\)/);
