...
`lib/fast-session-setting.ts` is the single source of truth for the session Fast preference. Stateless protocol and clone logic lives in `lib/fast-mode.ts`; explicit provider authorization lives in `lib/fast-mode-capabilities.ts`. Fast requires both an allowlisted provider and `api: "openai-responses"` or `api: "openai-completions"`. Add a provider only after confirming it accepts and forwards `service_tier: "priority"`; OpenAI-compatible transport alone is insufficient. The setting persists `pi-web-fast-mode-state` custom entries and serializes toggle/model-switch/reload application with last-write-wins semantics. Never mutate `inner.model` or a `modelRuntime` catalog object: the setting stores only model identity, resolves the latest catalog model inside its queue, and passes a session-private clone with an independent `compat`/`samplingParams` object to `setModel`. Disabling Fast must rebuild from the catalog model so an explicitly configured base `service_tier` is restored.
