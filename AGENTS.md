...
### Async session title generation must never run in parallel with the main agent
- Auto-naming (`POST /api/sessions/[id]/auto-name`) spawns a temporary `Agent` that reuses the source session's `streamFunction`/`transport`. Running it while the main agent is streaming would race the shared transport and can interrupt the main tool loop (leaving an unfinished `toolCall`).
- The route therefore subscribes to the wrapper's `agent_start` and aborts title generation the moment the main session starts a new run (`AbortSignal` passed into `generateSessionTitle`). The abort is reported to the UI as `session_title_skipped`, which `AppShell` treats as a silent idle reset, never as an error.
- `generateSessionTitle(source, signal?)` in `lib/session-title.ts` races the abort signal against both the idle wait (15 s) and the title run (90 s); on abort it stops the temporary agent and rejects with `AbortError`. It also strips images and truncates the context at full-turn boundaries (`limitTitleMessages`, 256 KB / 60 messages) so huge sessions with base64 screenshots cannot exceed the model request payload limit (413).

### File-level title generation (trash + bulk) does not create RPC sessions
- `generateTitleForSessionFile(filePath)` in `lib/session-file-title.ts` titles any session `.jsonl` (active or trashed) without building an `AgentSessionWrapper`: it creates SDK services + an `AgentSession` directly, runs `generateSessionTitle`, writes the result back via `setSessionName` (a `session_info` entry), then `dispose()`. No extension binding, no registry entry, no settings persistence. It skips files that already have a `session_info` name and sessions without any user message.
...
- Bulk backfill lives in `lib/bulk-title.ts` (`POST/GET/DELETE /api/titles/bulk`, no UI button — it was requested as a one-off backfill and the button was later removed, but the API remains for curl-triggered runs). It snapshots trash + all active sessions, runs sequentially (never parallel — avoids hammering providers), skips titled/empty sessions, tracks progress in `globalThis.__piBulkTitle` (hot-reload safe), and supports cooperative cancellation checked between files. Both generators guard the write with a `statSync` existence check so a file restored/purged mid-generation cannot be recreated as a stray empty shell by `appendFileSync`.
