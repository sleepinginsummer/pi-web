...
try {
  ...
  for (const viewport of [{ width: 1280, height: 800 }, { width: 390, height: 844 }]) {
    ...
    // Next dev cold-compiles the page and state route independently; allow both before judging hydration.
    const stateReady = page.waitForResponse(
      (response) => new URL(response.url()).pathname === `/api/sessions/${LONG}/state`,
      { timeout: 60_000 },
    );
    ...
    const latestUser = page.getByText(text(4998), { exact: true });
    ...
    if (viewport.width > 600) {
      ...
      const restoredOlderAnchor = await captureReadingAnchor();
      assert.ok(restoredOlderAnchor.entryId, "Returning to older history must keep rendered entries available");
      await selectSession("Render **E2E markdown**", "user");
